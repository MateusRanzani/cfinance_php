...
<section class="mb-6 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
    <form method="get" action="<?= $url('/despesas') ?>" class="grid gap-3 md:grid-cols-4">
...
<section class="mb-6 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
    ...
    <div class="mt-4 overflow-x-auto">
        <table class="w-full min-w-[980px] border-collapse text-sm">
            ...
            <tbody>
                <?php if ($expenses === []): ?><tr><td class="p-2 text-slate-500" colspan="7">Sem despesas no mes.</td></tr><?php endif; ?>
...
<section class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
    ...
    <form method="post" action="<?= $url('/fixos/despesas/add') ?>" class="mt-4 grid gap-3 md:grid-cols-6">
        <input type="hidden" name="csrf" value="<?= $e($csrf) ?>">
        <input type="hidden" name="mes" value="<?= $e($mesSelecionado) ?>">
        <input type="hidden" name="destino" value="despesas">
        <select class="rounded-lg border border-slate-300 px-3 py-2" name="tipo_id" required>
        ...
        <input class="rounded-lg border border-slate-300 px-3 py-2" type="number" step="0.01" min="0" name="valor_planejado" placeholder="Planejado" required>
        <input class="rounded-lg border border-slate-300 px-3 py-2" type="number" step="0.01" min="0" name="valor_real" placeholder="Real" required>
        <input class="rounded-lg border border-slate-300 px-3 py-2" type="number" min="1" max="31" name="dia_referencia" placeholder="Dia" value="10" required>
    ...
    <div class="mt-4 space-y-2">
        <?php if ($fixedExpenses === []): ?><p class="text-sm text-slate-500">Sem despesas fixas.</p><?php endif; ?>
        <?php foreach ($fixedExpenses as $item): ?>
            <form method="post" action="<?= $url('/fixos/despesas/delete') ?>" class="flex flex-wrap items-center gap-2 rounded-lg border border-slate-200 p-3 text-sm">
                <input type="hidden" name="csrf" value="<?= $e($csrf) ?>">
                <input type="hidden" name="mes" value="<?= $e($mesSelecionado) ?>">
                <input type="hidden" name="destino" value="despesas">
                <input type="hidden" name="id" value="<?= (int) $item['id'] ?>">
